	...
	public class ZipFile : IEnumerable, IDisposable
	{
		...
		public int FindEntry(string name, bool ignoreCase)
		{
			...
			for (int i = 0; i < entries_.Length; i++) {
				if (string.Compare(name.ToLower(), entries_[i].Name.ToLower()) == 0) {
					return i;
				}
			}
			#else
			for (int i = 0; i < entries_.Length; i++) {
				if (string.Compare(name, entries_[i].Name, ignoreCase, System.Globalization.CultureInfo.InvariantCulture) == 0) {
					return i;
				}
			}
			#endif
 ...
 */
...
#if !NET_1_0 && !NET_1_1 && !NETCF_1_0 && !UNITY_WINRT
			/// <summary>
	...
	public class StaticDiskDataSource : IStaticDataSource
	{
		...
		public Stream GetSource()
		{
			return null;// File.Open(fileName_, FileMode.Open, FileAccess.Read, FileShare.Read);
		}
	...
	public class DynamicDiskDataSource : IDynamicDataSource
	{
		...
		public Stream GetSource(ZipEntry entry, string name)
		{
			...
			if ( name != null ) {
				result = null;//File.Open(name, FileMode.Open, FileAccess.Read, FileShare.Read);
			}
