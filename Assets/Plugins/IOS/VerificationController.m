...
{    
    ...
    NSString *payload = [NSString stringWithFormat:@"{\"receipt-data\" : \"%@\", \"password\" : \"%@\"}",
                         jsonObjectString, baseKey];
    
    NSData *payloadData = [payload dataUsingEncoding:NSUTF8StringEncoding];
    
    NSURL* serverURL;
    if (testing) serverURL = [NSURL URLWithString:@"https://sandbox.itunes.apple.com/verifyReceipt"];
    else serverURL = [NSURL URLWithString:@"https://buy.itunes.apple.com/verifyReceipt"];
    ...
    currentCompletionHandler = [completionHandler copy];
...
#pragma pack(pop)
    ...
    CC_SHA1_Update(&sha1_ctx, purchase_info_bytes, (unsigned int)purchase_info_length);
