...
namespace Reign.EditorTools
{
	...
	{
		private const string checkURL = "http://www.reign-studios-services.com/ReignUnityPluginUpdater/4/WebPluginPackageValidateOwnership.aspx";
		private const string downloadURL = "http://www.reign-studios-services.com/ReignUnityPluginUpdater/4/WebPluginPackageDownload.aspx";
		...
		static void CheckForUpdates()
		{
			...
			using (var www = new WWW("http://www.reign-studios-services.com/ReignUnityPluginUpdater/4/WebPluginCheckForUpdates.aspx?Version=" + buildVersion))
			{
				...
				if (www.text == "CurrentVersion")
				{
					...
					EditorUtility.DisplayDialog("Current Version", "You are up to date!", "OK");
				}
		...
		[MenuItem("Edit/Reign/Download UnityPackage Updates/Download Windows package")]
		static void DownloadWindows()
		{
			string versionCheck = Application.dataPath+"/Plugins/Reign/VersionInfo/ReignVersionCheck_Windows";
			if (File.Exists(versionCheck))
			{
				using (var stream = new FileStream(versionCheck, FileMode.Open, FileAccess.Read, FileShare.Read))
				using (var reader = new StreamReader(stream))
				{
					int versionSyncCode = int.Parse(reader.ReadLine());
					string key = reader.ReadLine();
					if (validateKey(key)) downloadFile(key, "Windows");
				}
			}
			else
			{
				Debug.LogError("You dont own or have not installed the Windows Reign Unity package first.");
			}
		}
		...
		static void DownloadUltimate()
		{
			...
			// download
			if (validCount == 4) downloadFile("AE00088D-9EB4-48DB-BF5A-7AFB3C88F3EF", "Ultimate");
